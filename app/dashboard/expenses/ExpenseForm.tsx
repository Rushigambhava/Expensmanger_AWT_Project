...
export default function ExpenseForm({
    ...
    // Also need filtered subcategories
    ...
    const defaultDate = initialData?.ExpenseDate
        ? new Date(initialData.ExpenseDate.getTime() - initialData.ExpenseDate.getTimezoneOffset() * 60000).toISOString().slice(0, 16)
        : '';

    return (
        <div className="max-w-2xl mx-auto">
            <div className="mb-6">
                <Link href="/dashboard/expenses" className="text-slate-500 hover:text-slate-800 flex items-center mb-2">
                    <ArrowLeft className="w-4 h-4 mr-1" /> Back to Expenses
                </Link>
                <h2 className="text-2xl font-bold text-slate-800">{title}</h2>
                <p className="text-slate-500">Record transaction details.</p>
            </div>
            ...
            <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100">
                <form action={action} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                key={selectedCategory} // Force re-render/reset when category changes
                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label htmlFor="peopleId" className="block text-sm font-medium text-slate-700 mb-1">People / Beneficiary</label>
                            <select
                                id="peopleId"
                                name="peopleId"
                                className="w-full px-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-white"
